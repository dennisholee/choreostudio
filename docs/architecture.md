...
The public schema lives under [`docs/schema/`](./schema/README.md). Use [`docs/schema/canvas.schema.json`](./schema/canvas.schema.json) for the persistence contract and `x-merge-class` annotations, and use this guide for the runtime interpretation of that schema.
